...
ERROR_ANALYSIS_REPORT_PATH = REPORTS_DIR / "error_analysis_report.md"
INFERENCE_BENCHMARK_PATH = REPORTS_TABLES_DIR / "inference_benchmark.csv"
ERROR_BY_RECORD_PATH = REPORTS_TABLES_DIR / "error_analysis_by_record.csv"
ERROR_BY_SLEEP_STAGE_PATH = REPORTS_TABLES_DIR / "error_analysis_by_sleep_stage.csv"
...
- `{relative(INFERENCE_BENCHMARK_PATH)}`: inference latency, memory, and artifact-size benchmark.
- `{relative(ERROR_BY_RECORD_PATH)}` and `{relative(ERROR_BY_SLEEP_STAGE_PATH)}`: FP/FN breakdown by record and sleep stage.
...
- `{relative(PERFORMANCE_REPORT_PATH)}`: engineering performance and deployment notes.
- `{relative(ERROR_ANALYSIS_REPORT_PATH)}`: record-level FP/FN error analysis.
