...
CV_PREDICTIONS_PATH = REPORTS_TABLES_DIR / "temporal_ensemble_cv_predictions.csv"
...
def make_prediction_table(
    ...
    id_columns = ["record_id", "epoch_id", "fold", "label_binary"]
    rows: list[pd.DataFrame] = []

    for (feature_set, model_name, postprocessing, window, centered), scores in experiments.items():
        table = predictions[id_columns].copy()
        table["regime"] = "sleep_only"
        table["feature_set"] = feature_set
        table["model"] = model_name
        table["postprocessing"] = postprocessing
        table["smoothing_window_epochs"] = int(window)
        table["smoothing_centered"] = bool(centered)
        table["y_proba"] = scores.to_numpy(dtype=float)
        table["y_pred_0_50"] = (table["y_proba"] >= THRESHOLD).astype(int)
        rows.append(table)
...
def log_mlflow_summary(
    ...
    with mlflow_run(
        ...
        params = {
            ...
            "n_rows_sleep_only": len(sleep_features),
            "n_records": sleep_features["record_id"].nunique(),
            "n_spo2_features": len(feature_sets["spo2_enhanced"]),
            "n_flow_spo2_features": len(feature_sets["flow_spo2_enhanced"]),
            "previous_best_auc": PREVIOUS_BEST_AUC,
            "previous_best_f1": PREVIOUS_BEST_F1,
            "previous_tuned_f1": PREVIOUS_TUNED_F1,
        }
        if best_auc is not None:
            params.update(
                {
                    "best_auc_feature_set": best_auc["feature_set"],
                    "best_auc_model": best_auc["model"],
                    "best_auc_postprocessing": best_auc["postprocessing"],
                    "best_auc_smoothing_window_epochs": best_auc[
                        "smoothing_window_epochs"
                    ],
                    "best_auc_smoothing_centered": best_auc["smoothing_centered"],
                }
            )
        if best_tuned is not None:
            params.update(
                {
                    "best_tuned_feature_set": best_tuned["feature_set"],
                    "best_tuned_model": best_tuned["model"],
                    "best_tuned_postprocessing": best_tuned["postprocessing"],
                    "best_tuned_threshold": best_tuned["threshold"],
                }
            )
            ...
            metrics.update(
                {
                    ...
                    "best_auc_f1_mean": best_auc["f1_mean"],
                    "best_auc_sensitivity_mean": best_auc["sensitivity_mean"],
                    "best_auc_specificity_mean": best_auc["specificity_mean"],
                    "best_auc_average_precision_mean": best_auc[
                        "average_precision_mean"
                    ],
                    "best_roc_auc_delta_vs_previous": best_auc["roc_auc_mean"]
                    - PREVIOUS_BEST_AUC,
                }
            )
            ...
            metrics.update(
                {
                    ...
                    "best_f1_roc_auc_mean": best_f1["roc_auc_mean"],
                    "best_f1_delta_vs_previous": best_f1["f1_mean"]
                    - PREVIOUS_BEST_F1,
                }
            )
        if best_tuned is not None:
            metrics.update(
                {
                    "best_tuned_f1": best_tuned["f1"],
                    "best_tuned_sensitivity": best_tuned["sensitivity"],
                    "best_tuned_specificity": best_tuned["specificity"],
                    "best_tuned_precision": best_tuned["precision"],
                    "best_tuned_f1_delta_vs_previous": best_tuned["f1"]
                    - PREVIOUS_TUNED_F1,
                }
            )
...
def main() -> None:
    ...
    cv_predictions = make_prediction_table(predictions, experiments)
